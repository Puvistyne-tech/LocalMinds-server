...
- data: The post data or post ID (for DELETE)
    ...
    schema: {
      ...
      properties: {
        ...
        data: {
          ...
          description:
            "Event payload - full post object for ADD/UPDATE, post ID for DELETE",
        },
      },
      example: {
        type: "POST_ADD",
  ...
  postUpdates(): Observable<any> {
    const eventSource = fromEvent(this.postsService.eventEmitter, "postEvent");
    return eventSource.pipe(map((event) => event as MessageEvent));
  }
  ...
  async findPostsByCriteria(
    ...
    @Query("location") location?: LocationQueryDto,
  ...
  @Get("nearby")
  @ApiOperation({ summary: "Find posts near a location" })
  @ApiResponse({
    status: 200,
    description: "Returns posts within the specified radius",
  })
  async findNearby(@Query() locationQuery: LocationQueryDto) {
    return this.postsService.findNearby(locationQuery);
  }

  @Get("bounds")
  @ApiOperation({ summary: "Find posts within map bounds" })
  @ApiResponse({
    status: 200,
    description: "Returns posts within the specified bounds",
  })
  async findWithinBounds(
    ...
    return this.postsService.findWithinBounds(swLat, swLng, neLat, neLng);
  }
}
